...
class FootballData {
    ...
    public function findMatchesForDateRangeByCompetition($id,$start,$end)
    {
        $resource = 'competitions/' . $id . '/matches/?dateFrom=' . $start . '&dateTo=' . $end;
        $response = file_get_contents($this->baseUri . $resource, false, 
                                      stream_context_create($this->reqPrefs));
    ...
    public function findScorersByCompetition($id) {
        $resource = 'competitions/' . $id . '/scorers';
            $response = file_get_contents($this->baseUri . $resource, false, 
                                          stream_context_create($this->reqPrefs));
            return json_decode($response);
        }
    ...
    public function findMatchesByTeam($teamId) {
        $resource = 'teams/' . $teamId . '/matches';
    ...
    /**
     * Function returns one unique player identified by a given id.
     * 
     * @param int $id
     * @return stdObject player
     */    
